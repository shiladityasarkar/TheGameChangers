...
class GameTasks():
	def code_task(self, agent, game):
    	...
    	{game}
			...
			"""),
			agent=agent,
			expected_output = 'Full working python code that creates the game asked by the user.'
		)
	...
	def review_task(self, agent, game):
		return Task(description=dedent(f"""\
			...
			"""),
			agent=agent,
			expected_output = 'Correct python code that creates and runs the game.'
		)
	...
	def evaluate_task(self, agent, game):
		return Task(description=dedent(f"""\
			...
			"""),
			agent=agent,
			expected_output = 'Ensure that the python code is correct and complete. It must produce the game as asked.'
		)